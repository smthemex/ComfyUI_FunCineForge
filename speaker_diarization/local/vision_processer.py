...
import os, time, torch, cv2, pickle, gc
import python_speech_features
from scipy import signal
import threading
import queue
from contextlib import contextmanager
from .vision_tools import face_detection as face_detection
from .vision_tools import active_speaker_detection as active_speaker_detection
from  .vision_tools import face_recognition as face_recognition
from .vision_tools import face_quality_assessment as face_quality_assessment
from .vision_tools import lip_detection as lip_detection
# import vision_tools.lip_encoder as lip_encoder

class ModelPool:
    def __init__(self, creator, pool_size=1):
        """
        creator: callable that creates and returns a model instance
        pool_size: int
        """
    ...
    def borrow(self, timeout=None):
        """
        Borrow a model instance; returns a context manager.
        timeout: seconds to wait; 如果为 None 则阻塞直到可得
        """
        try:
            inst = self._q.get(timeout=timeout)
        except queue.Empty:
            raise RuntimeError("Timeout when borrowing a model instance from pool")
        try:
            yield inst
        finally:
            # 把实例返回到队列
...
class SharedModels:
    def __init__(self, onnx_dir, device='cpu', device_id=0, pool_sizes=None):
        print(f"device, {device}")
        # create pools for each model type
        self.face_pool = ModelPool(lambda: face_detection.Predictor(onnx_dir, device, device_id),
                                   pool_size=pool_sizes.get('face', 1))
        self.asd_pool = ModelPool(lambda: active_speaker_detection.ASDTalknet(onnx_dir, device, device_id),
                                   pool_size=pool_sizes.get('asd', 1))
        self.fq_pool = ModelPool(lambda: face_quality_assessment.FaceQualityAssess(onnx_dir, device, device_id),
                                   pool_size=pool_sizes.get('fq', 1))
        self.fr_pool = ModelPool(lambda: face_recognition.FaceRecIR101(onnx_dir, device, device_id),
                                   pool_size=pool_sizes.get('fr', 1))
        self.lip_pool = ModelPool(lambda: lip_detection.LipDetector(onnx_dir, device, device_id),
                                   pool_size=pool_sizes.get('lip', 10))

    def detect_faces(self, image, top_k=10, prob_threshold=0.9, borrow_timeout=1000):
        with self.face_pool.borrow(timeout=borrow_timeout) as model:
...
class VisionProcesser():
    ...
    def run(self):
        ...
        out_data = {
            ...
            'frameI': self.active_facial_embs['frameI'],
            'faceI': self.active_facial_embs['faceI'], # 说话人人脸帧索引
    ...
    def face_detection(self, frames):
        ...
        for fidx, image in enumerate(frames):
            ...
            bboxes, _, probs = self.shared_models.detect_faces(image_input, top_k=10, prob_threshold=0.9)
    ...
    def evaluate_asd(self, tracks):
        ...
        for ins in tracks:
            ...
            score = self.shared_models.asd_score(audio_feature, video_feature)
    ...
    def evaluate_fr(self, frames, tracks, scores):
        ...
        for fidx in range(0, len(faces), max(1, self.face_det_stride)):
            ...
            res = self.shared_models.detect_lip(best['facedata'])
            # 如果没有检测到嘴唇，跳过，这里已经筛去了少数低质量像素的人脸
            ...
            feature, feature_normalized = self.shared_models.get_face_embedding(best['facedata'])
            # active_facial_embs['feat'].append(feature_normalized) # 归一化面部特征（身份信息）
            ...
            if s < OFF_THRESHOLD:
                continue
            # 人脸质量评估（可选，开启后只会筛选评分高的人脸帧）
            # face_q_score = self.shared_models.face_quality_score(best['facedata'])
            # if (face_q_score >= QUALITY_HIGH) or (face_q_score >= QUALITY_LOW and s >= ON_THRESHOLD):
            if  s >= OFF_THRESHOLD:
                # feature, feature_normalized = self.shared_models.get_face_embedding(best['facedata']) # 仅保留模型认为在说话帧
                # active_facial_embs['feat'].append(feature_normalized)
