...
def init_worker(local_dir, use_gpu, batch_size):
...
def main(args):
    ...
    # parser.add_argument('--local_dir', default='', type=str, help='HuggingFace access token (if required)')
    ...
    # 确保workers至少为1
    workers = 1
    print(f"[INFO] Overlap detection will process {len(wavs)} files using {workers} worker")

    # 初始化模型
    init_worker(args.local_dir, args.use_gpu, args.batch_size)

    # 单线程顺序处理所有文件
    results = []
    with tqdm(total=len(wavs), desc="Processing", unit="file") as pbar:
        for wpath in wavs:
            res = process_one(wpath, args.overlap_threshold)
            results.append(res)
    ...
    # init_args = (args.local_dir, args.use_gpu, args.batch_size)
