...
def main(args):
    ...
    # parser.add_argument('--world_size', type=int, default=0, help="world size or thread_num")

    # args = parser.parse_args()
    conf = yaml_config_loader(args.conf)
    rank = int(os.environ.get('LOCAL_RANK', -1))
    threads_num = int(os.environ.get('WORLD_SIZE', 1))

    disted = threads_num > 1 and len(args.gpu) > 1

    if rank < 0:
        rank = args.rank
        threads_num = args.world_size
    if rank <= 0:
        print(f"[INFO] Extract speech embs using {threads_num} threads, GPUs: {True if args.use_gpu else False}")
    ...
    if args.model_id is not None:
        ...
        conf['pretrained_model'] = os.path.join(args.pretrained_model, 'campplus_cn_en_common.pt')
