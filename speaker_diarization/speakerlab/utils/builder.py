...
def dynamic_import(import_path):
    speakerlab_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    parent_dir = os.path.dirname(speakerlab_dir)
    
    # 添加调试信息
    print(f"Current working directory: {os.getcwd()}")
    print(f"Speakerlab directory: {speakerlab_dir}")
    print(f"Parent directory: {parent_dir}")
    print(f"Import path: {import_path}")
    print(f"Current sys.path: {sys.path[:3]}")  # 打印前3个路径
    
    for dir_path in [speakerlab_dir, parent_dir]:
...
# def dynamic_import(import_path):
#     speakerlab_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
