...
from ..models.modules.hifigan import init_weights
from ..models.modules.hifigan.activations import Snake
...
class SourceModuleHnNSF(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        noise_source (batchsize, length 1)
        """
        dtype = self.l_linear.weight.dtype  
        device = self.l_linear.weight.device  
        # source for harmonic branch
        with torch.no_grad():
            sine_wavs, uv, _ = self.l_sin_gen(x)
        if sine_wavs.dtype != dtype:
            sine_wavs = sine_wavs.to(dtype=dtype, device=device)
...
class CausalHiFTGenerator(nn.Module):
    ...
    def _istft(self, magnitude, phase):
        ...
        real = magnitude_f32 * torch.cos(phase_f32)
        img = magnitude_f32 * torch.sin(phase_f32)
