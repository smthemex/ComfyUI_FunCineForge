...
def load_model(llm,flow,voc,kwargs,dtype="bf16"):
    lm_ckpt_path=folder_paths.get_full_path("funcineforge", llm) if llm != "none" else None
    fm_ckpt_path=folder_paths.get_full_path("funcineforge", flow) if flow != "none" else None
    voc_ckpt_path=folder_paths.get_full_path("funcineforge", voc) if voc != "none" else None
    
    logger = get_logger(log_level=logging.INFO, local_rank=1, world_size=1)
    ...
    params={k: v for k, v in kwargs.items() if k in ['face_encoder_conf_local', 'tokenizer_conf_local','model_conf_local',"mode","infer_dtype"]}
    ...
    lm_model.model.to(dtype_map[dtype])
    kwargs["mode"]="flow"
    # build FM model
    fm_exp_dir, fm_model_name, fm_ckpt_id = extract_path_components(fm_ckpt_path)
    ...
    fm_model.model.to(dtype_map[dtype])

    # build voc model
    voc_exp_dir, voc_model_name, voc_ckpt_id = extract_path_components(voc_ckpt_path)
    ...
    voc_model.model.to(dtype_map[dtype])
    
     # build inference model
    logger.info(f"build inference model {kwargs.get('model')}")
    kwargs["output_dir"] = folder_paths.get_output_directory()
