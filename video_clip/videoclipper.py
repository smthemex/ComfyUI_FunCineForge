...
def init_models(lang='zh', device='cpu',model_dict={}):
    """初始化模型，使用缓存避免大量下载"""
    ...
    if lang == 'zh':
        if model_dict:
            model = AutoModel(**model_dict,device=device)
        else:
            model = AutoModel(
...
def _init_worker(lang: str, device: str, model_dict: dict):
    ...
    if _process_local is None:
        ...
        if device == 'cuda' and torch.cuda.is_available():
            ...
            _process_local = {'model': init_models(lang, f'cuda:{0}',model_dict)}
        else:
            _process_local = {'model': init_models(lang, 'cpu',model_dict)}
...
def process_single_video(video_file, stage, sd_switch, base_output_dir, lang):
    try:
        ...
        print(f"✅ [Worker- base_output_dir: {base_output_dir},{parent_dir_name},{video_name}")
        output_dir = os.path.join(base_output_dir, parent_dir_name, video_name)
        print(f"✅ [Worker- 创建输出目录: {output_dir}")
        os.makedirs(output_dir, exist_ok=True)
...
def main(args):
    ...
    print(f"[INFO] Processing {file_or_folder}")
    stage = kwargs['stage']
    ...
    _init_worker(lang, device,kwargs["model_dict"])
    ...
    for file in all_videos:
        process_single_video(file, stage, sd_switch, output_dir, lang)
    # # 多进程处理
    # if stage == 1:
