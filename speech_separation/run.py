...
def main(args):
    ...
    # args =p.parse_args()
    # 收集所有 wav
    all_wavs,has_processed = find_wavs(args.root)
    if len(all_wavs) == 0:
        if has_processed>0:
            print(f"All wavs have been processed {has_processed}. No need to process again.")
            return
        else:
    ...
    if args.gpus is not None and torch.cuda.is_available():
        # 使用GPU单线程处理
        device_ids = args.gpus if isinstance(args.gpus, list) else [args.gpus]
        device_id = device_ids[0]  # 使用第一个GPU
        print(f"Using GPU {device_id} for processing...")
        process_track_list(
            device_id, 
            args.model_type, 
            args.config_path, 
